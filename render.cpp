...
const unsigned int gSongStartSample =
    88200;  // specify real value here, 0 is the very first sample in the file
// marker for when the song ends
const char* gSongEndMarker = "song_end";
// the sample index for the end of the song
const unsigned int gSongEndSample =
    5292000;  // specify real value here, placeholder is 44,100 * 120 (2
              // minutes)
...
// one of them loads the next chunk of audio while the other one is used for
// playback
...
const unsigned int gTickIntervalSamples =
    gTickInterval * gBaseSampleRate / 1000;
...
enum Event { kPlaybackStart = 0, kPlaybackEnd, kSongStart, kSongEnd };
...
const std::vector<const char*> gEventLabels = {
    gPlaybackStartMarker, gPlaybackEndMarker, gSongStartMarker, gSongEndMarker};
...
void fillBuffer(void*) {
  ...
  if (gBufferReadPtr >= gNumFramesInFile) gBufferReadPtr = 0;
  ...
  for (unsigned int ch = 0; ch < gSampleBuf[0].size(); ++ch) {
    ...
    AudioFileUtilities::getSamples(gFilename,
                                   gSampleBuf[!gActiveBuffer][ch].data(), ch,
...
void eventLabelMarker(void*) {
  // if we use output buffering, flush any previous messages before sending the
  // label
  if (gPrintEveryNTicks > 0) {
    fflush(stdout);
  }
...
bool setup(BelaContext* context, void* userData) {
  ...
  if (gNumFramesInFile <= 0) return false;

  if (gNumFramesInFile <= BUFFER_LEN) {
    printf(
        "Sample needs to be longer than buffer size. This example is intended "
        "to work with long samples.");
    return false;
  }

  // load the file up to the specified buffer length
  gSampleBuf[0] = AudioFileUtilities::load(gFilename, BUFFER_LEN, 0);
  gSampleBuf[1] = gSampleBuf[0];  // initialise the inactive buffer with the
                                  // same channels and frames as the first one

  // initialise the QTM realtime protocol
  rtProtocol = new CRTProtocol();
  // Connect to the QTM application
  if (!rtProtocol->Connect(serverAddr, basePort, &nPort, majorVersion,
...
void render(BelaContext* context, void* userData) {
  ...
  for (unsigned int n = 0; n < context->audioFrames; n++) {
    ...
    if (++gReadPtr >= BUFFER_LEN) {
      // if the file hasn't finished reading, request the next buffer to be
      // filled
      if (!gDoneReadingFile) {
        if (!gDoneLoadingBuffer)
          rt_printf(
              "\nCouldn't load buffer in time :( -- try increasing buffer "
              "size!\n");
        ...
        // if we're done reading, and we're at the final buffer, request program
        // stop
    ...
    if (gEventStatus[Event::kPlaybackStart] != EventStatus::kEventConfirmed &&
        gPlaybackIndex >= 0) {
    ...
    if (gEventStatus[Event::kSongStart] != EventStatus::kEventConfirmed &&
        gPlaybackIndex >= gSongStartSample) {
      // if we haven't already requested the label, send it now
    ...
    // the order is of comparison is switched for fast failing (more likely to
    // be false for longer)
    if (gPlaybackIndex >= gSongEndSample &&
        gEventStatus[Event::kSongEnd] != EventStatus::kEventConfirmed) {
      // if we haven't already requested the label, send it now
    ...
    if (gEventRequested) {
      // if we have requested an event, schedule the task to send the event
      // label to QTM
      Bela_scheduleAuxiliaryTask(gEventLabelMarkerTask);
    }
